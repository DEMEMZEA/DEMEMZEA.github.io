...
;(function (globalScope) {
    ...
    P.times=P.mul=P.mult=function (other){
      var x=this.clone();
      other=new ExpantaNum(other);
      if (ExpantaNum.debug>=ExpantaNum.NORMAL) console.log(x+"*"+other);
    ...
    Q.times=Q.mul=Q.mult=function (x,y){
    ...
    Q.softcap=Q.cap=function (x,start,amount,mode){
    return new ExpantaNum(x).cap(start,amount,mode)
    }
    P.softcap=P.cap=function(start,amount,mode=0){
    start = new ExpantaNum(start)
    let x = this.clone()
    if(x.lt(start)) return x
    if(mode===0||mode==="sub"){
    return x.sub(start).div(amount).add(start)
    }
    if(mode===1||mode==="div"){
    return x.div(start).pow(amount).mul(start)
    }
    }
    //end break_eternity.js excerpt
